...
//change all functions with Printer& return value to noinline
//may or may not help in code size
#define self [[ gnu::noinline ]] Printer
...
namespace prn { //so can << bufprinter  and get the buffer printed out
                ...
                template<unsigned N>
                inline Printer&
                operator<<(Printer& p, BufPrinter<N>& b)
...
setwmax         (int n) { return {n}; }
                inline Printer&
                operator<<(Printer& p, SetwMax_prn s)
...
setfill         (char c = ' ') { return {c}; }
                inline Printer&
                operator<<(Printer& p, Setfill_prn s)
                { return p.fill(s.c); }

                enum ENDL_prn {
endl            };
                inline Printer&
                operator<<(Printer& p, ENDL_prn e)
...
hex             = 16 };
                inline Printer&
                operator<<(Printer& p, BASE_prn e)
                { return p.base(e); }

                enum CLEAR_prn {
clear           };
                inline Printer&
                operator<<(Printer& p, CLEAR_prn e)
...
showpos         };
                inline Printer&
                operator<<(Printer& p, POSITIVE_prn e)
                { return p.positive(e); }
...
showalpha       };
                inline Printer&
                operator<<(Printer& p, ALPHA_prn e)
...
left            };
                inline Printer&
                operator<<(Printer& p, JUSTIFY_prn e)
...
uppercase       };
                inline Printer&
                operator<<(Printer& p, UPPERCASE_prn e)
...
showbase        };
                inline Printer&
                operator<<(Printer& p, SHOWBASE_prn e)
                { return p.showbase(e); }
...
#define Hex0            Hex << showbase
#define bin0            bin << showbase
#define endlc           endl << clear
#define chardup(c,n)    setwf(n,c) << ""
